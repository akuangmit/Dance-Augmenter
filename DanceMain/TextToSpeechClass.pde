// the text to speech class, used for voice output from the system to the user.
...
static class TextToSpeech extends Object {
  ...
  // A new thread is spun up for voice output
  static void say(String script, String voice, int speed) {
    try {
      //ProcessBuilder builder = new ProcessBuilder();
      //Process pro = builder.start(); 
      //pro.exec(new String[] {"say", "-v", voice, "[[rate " + speed + "]]" + script});
      Process currentProcess = Runtime.getRuntime().exec(new String[] {"say", "-v", voice, "[[rate " + speed + "]]" + script});
